...
def sort_category_dict_out_of_wine_dict(category_dict_out_of_wine_dict):
    ...
    for element in sorted(category_dict_out_of_wine_dict.keys()):
        sorted_category_dict_out_of_wine_dict[element] = category_dict_out_of_wine_dict[element]
...
def proper_translation_of_year_into_russian_on_basis_of_company_age(year):
    ...
    if last_number_of_age_of_company in special_condition_numbers_for_god and year not in exceptions:
        proper_word_in_russian = proper_words_in_russian_dict[1]
    elif last_number_of_age_of_company in special_condition_numbers_for_goda and year not in exceptions:
...
if __name__ == "__main__":
    ...
    sorted_category_dict_out_of_wine_dict = sort_category_dict_out_of_wine_dict(category_dict_out_of_wine_dict)
    age_of_company = count_age_of_company(YEAR_OF_INCORPORATION)
    proper_word_in_russian = proper_translation_of_year_into_russian_on_basis_of_company_age(age_of_company)
